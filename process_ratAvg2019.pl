...
use civm_simple_util qw(activity_log load_file_to_array write_array_to_file get_engine_constants_path printd file_update file_mod_extreme whoami whowasi debugloc sleep_with_countdown $debug_val $debug_locator);# debug_val debug_locator);
...
($jnk,my $label_file_name)=fileparts($in_color,3);
...
# We use this updated lookup table for the abbreviations and name versions.
{
    ...
    my ($cp,$cn,$ce)=fileparts($in_color,3);
    $out_color=File::Spec->catfile($cp,"${cn}_${rt}_out$ce");

    #$out_color='D:/Libraries/Brain/Rattus_norvegicus/Wistar/151124_3_1/v2019-04-18/scalars/atlas/labels/xmas2015rat_symmetric_croppedxmas2015_labels_20190614_atlas_lookup_Structure_out.txt';
    ##
    ## NAUGHTY OUT COLOR NOT CONFIGURABLE
    ##
    ...
    #
    use File::Compare;
    # Forcing compare success because on inspection, it was only the color comment lines
    # which were different.
    if (1 || compare($in_color,$out_color) == 0) {
    ...
    #
    # Stage 2 generates the name identified mrml
    $rt="Name";
